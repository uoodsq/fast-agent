...
from mcp_agent.llm.providers.augmented_llm_github import GitHubAugmentedLLM
...
class ModelFactory:
    ...
    PROVIDER_CLASSES: Dict[Provider, LLMClass] = {
        ...
        Provider.GITHUB: GitHubAugmentedLLM,
    ...
    def parse_model_string(cls, model_string: str) -> ModelConfig:
        ...
        if len(parts_for_provider_model) >= 2:
            potential_provider_str = (
                f"{parts_for_provider_model[0]}.{parts_for_provider_model[1]}"
            )
        ...
        if identified_provider_parts > 0:
            model_name_str = ".".join(
                parts_for_provider_model[identified_provider_parts:]
            )
        else:
